Many others have contributed to the <B>BRLTTY</B> project.
They include:
<DL>
<?php contributor("Nikhil Nair", "nn201@cus.cam.ac.uk"); ?>
<UL>
<LI>Original author.
<LI>Author of driver for <CITE>BrailleLite</CITE> from <A HREF="http://www.blazie.com/pages/main.html">Blazie Engineering</A>.
<LI>Author of driver for <CITE>CombiBraille</CITE> from <A HREF="http://www.braillevoyager.nl/uk/index.html">Tieman B.V.</A>
<LI>Author of driver for <CITE><A HREF="http://www.cstr.ed.ac.uk/projects/festival/">Festival</A></CITE> text to speech engine from <A HREF="http://www.ed.ac.uk/">The University of Edinburgh</A> (<A HREF="http://www.cstr.ed.ac.uk/">Centre for Speech Technology Research</A>).
</UL>
<?php contributor("James Bowden"); ?>
<UL>
<LI>Former project advisor.
<LI>Contributor of some braille translation tables.
</UL>
<?php contributor("August Hörandl", "hoerandl@elina.htlw1.ac.at"); ?>
<UL>
<LI>Author of driver for <CITE>Screen 2D Terminal</CITE> from <A HREF="http://www.papenmeier.de/">Papenmeier</A>.
Thanks to the members of his team
(Tibor Becker, Michael Burger, Herbert Gruber, and Heimo Schön)
who worked on this project at the
<A HREF="http://www.htlw16.ac.at/">Technical High School</A>,
<A HREF="http://elina.htlw16.ac.at/">Department of Electrical Engineering</A>,
Vienna, Austria.
<LI>Initial run-time loadable driver module design.
</UL>
<?php contributor("Oscar Fernandez", "ofa@once.es"); ?>
<UL>
<LI>Author of driver for <CITE>EcoBraille</CITE> from <A HREF="http://cidat.once.es/">La O.N.C.E.</A>
</UL>
<?php contributor("Jos Lemmens", "jlemmens@inter.nl.net"); ?>
<UL>
<LI>Provider of parallel port support library for <CITE>Alva</CITE> from <A HREF="http://www.alva-bv.nl/alvacorp/alva_corp_home.html">Alva B.V.</A>
</UL>
<?php contributor("Yannick Plassiard", "Yannick.Plassiard@free.fr"); ?>
<UL>
<LI>Maintainer of driver for <CITE>EuroBraille</CITE> from <A HREF="http://www.eurobraille.fr/">EuroBraille</A>.
</UL>
<?php contributor("Brailcom o.p.s.", "technik@brailcom.cz", "http://www.brailcom.cz/"); ?>
<UL>
<LI>Author of driver for <CITE>MiniBraille</CITE> from <A HREF="http://www.braillevoyager.nl/uk/index.html">Tieman B.V.</A>
</UL>
<?php contributor("Wolfgang Astleitner", "wolfgang.astleitner@liwest.at"); ?>
<UL>
<LI>Author of driver for <CITE>MultiBraille</CITE> from <A HREF="http://www.braillevoyager.nl/uk/index.html">Tieman B.V.</A>
</UL>
<?php contributor("Mario Lang", "mlang@delysid.org"); ?>
<UL>
<LI>Author of driver for <CITE>Vario/RBT</CITE> (native mode) from <A HREF="http://www.baum.de/English/homeeng1.htm">Baum</A>.
<LI>Author of driver for <CITE><A HREF="http://www.speech.cs.cmu.edu/flite/index.html">Festival Lite</A></CITE> text to speech engine from <A HREF="http://www.cmu.edu">Carnegie Mellon University</A> (<A HREF="http://www.cs.cmu.edu">School of Computer Science</A>).
</UL>
<?php contributor("Per Lejontand", "pele@acc.umu.se"); ?>
<UL>
<LI>Author of driver for <CITE>Vario/RBT</CITE> (emulation 2) from <A HREF="http://www.baum.de/English/homeeng1.htm">Baum</A>.
</UL>
<?php contributor("Sébastien Hinderer", "sebastien.hinderer@libertysurf.fr"); ?>
<UL>
<LI>Author of driver for <CITE>VisioBraille</CITE> from <A HREF="http://www.handialog.com/indexuk.htm">Handialog</A>.
<LI>Co-author of Application Programming Interface.
</UL>
<?php contributor("Christian Comaschi", "christian_comaschi@libero.it"); ?>
<UL>
<LI>Author of driver for <CITE>VideoBraille</CITE> from <A HREF="http://www.tinlecco.it/tiflosoft">Tiflosoft</A>.
</UL>
<?php contributor("Andreas Gross", "andi@andi-bika.de"); ?>
<UL>
<LI>Author of driver for <CITE>modular displays</CITE> from <A HREF="http://www.handytech.com/">Handy Tech Elektronik GmbH</A>.
</UL>
<?php contributor("Stephane Dalton", "sdalton@videotron.ca"); ?>
<UL>
<LI>Designer of kernel module for <CITE>Voyager</CITE> from <A HREF="http://www.braillevoyager.nl/uk/index.html">Tieman B.V.</A>.
</UL>
<?php contributor("John Boyer", "director@chpi.org"); ?>
<UL>
<LI>Author of support for in-line contracted braille.
</UL>
<?php contributor("Samuel Thibault", "samuel.thibault@labri.fr"); ?>
<UL>
<LI>Co-author of the Application Programming Interface.
<LI>Author of the Tty and XWindow test drivers.
</UL>
<?php contributor("Coscell Kao", "coscell@mail.batol.net"); ?>
<UL>
<LI>Author of the big5 contraction table.
</UL>
</DL>
